...
geschrieben. Die erste Spalte enthält den Dateinamen des Quell-JPGs, damit
die Herkunft jeder Zeile nachvollziehbar bleibt.
...
Zellzuordnung:
    ...
    als ein Tabulator-Trennzeichen.

Es findet KEINE Normierung der Spaltenzahl statt: Wenn das Modell für
eine Zeile weniger Schlüssel liefert, wird die CSV-Zeile entsprechend
kürzer (zusätzlich zur Quelldatei-Spalte).
...
def parse_model_output(raw_text: str) -> list[list[str]]:
    ...
    Es findet KEINE Normalisierung der Spaltenzahl statt.
    """
...
def transcribe_image(image_path: Path, api_key: str) -> list[list[str]]:
    ...
    Gibt eine Liste von Zeilen zurück (jede Zeile als Liste von Zellen).
    """
...
def main() -> int:
    ...
    # Übergeordnete Ordner für die Ausgabe-CSV anlegen, falls nötig
    ...
    # CSV im Streaming-Modus schreiben: nach jedem JPG werden dessen
    # Zeilen direkt angehängt. So gehen bei einem Absturz auf halber
    # Strecke die bisher transkribierten Daten nicht verloren.
    with open(args.output_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for idx, jpg in enumerate(jpg_files, start=1):
            ...
            # Jede Zeile mit dem Quelldateinamen voranstellen
            for row in rows:
            ...
            f.flush()  # auf die Platte zwingen, falls etwas abstürzt
